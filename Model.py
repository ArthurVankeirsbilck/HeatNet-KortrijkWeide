...
def objective_rule(model):
    return sum(model.C_gen[i, t] * model.P[p, i, t] + model.C_import[i, t] * model.I[i, t] + model.C_export[i, t] * model.E[i, t] - model.P_el[p,i,t]*0.4
...
def transmission_constraint_rule(model, t):
    return sum(model.E[i, t]*model.X[i] for i in model.N) <= model.P_transmission
...
def consistency_constraint_rule(model, i):
    return model.X[i] == sum(model.Z[i, j] for j in model.N if i != j)
model.consistency_constraint = Constraint(model.N, rule=consistency_constraint_rule)
