...
model.obj = Objective(expr=sum(model.p[i,t]*model.Cgen[i]  + model.Ppump[i,t] for i in model.N for t in model.T), sense=minimize)

def pumppower(model, i ,t):
    return model.Ppump[i,t] == 186.42*model.m_pipe[i,t] - 530.08
